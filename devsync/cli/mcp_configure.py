...
def mcp_configure_command(
    ...
    MCP servers. Credentials are stored in .devsync/.env (gitignored).
    ...
    try:
        ...
        if json_output:
            ...
            if install_scope == InstallationScope.GLOBAL:
                env_path = _resolve_data_dir(Path.home(), ".devsync", [".instructionkit"]) / "global" / ".env"
            else:
                env_path = _resolve_data_dir(Path.cwd(), ".devsync", [".instructionkit", ".ai-config-kit"]) / ".env"
...
def _get_library_root() -> Path:
    ...
    Returns:
        Path to library root (~/.devsync/library/)
    """
    return _resolve_data_dir(Path.home(), ".devsync", [".instructionkit"]) / "library"
