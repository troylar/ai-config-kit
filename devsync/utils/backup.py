...
def create_backup(file_path: Path, backup_dir: Optional[Path] = None) -> Path:
    ...
    Args:
        ...
        backup_dir: Optional custom backup directory (defaults to .devsync/backups)
    ...
    Example:
        ...
        >>> # Original file preserved at .devsync/backups/TIMESTAMP/company.test.md
    """
    ...
    if backup_dir is None:
        ...
        if project_root:
            backup_dir = _resolve_data_dir(project_root, ".devsync", [".instructionkit", ".ai-config-kit"]) / "backups"
        else:
            backup_dir = _resolve_data_dir(Path.home(), ".devsync", [".instructionkit"]) / "backups"
...
def restore_backup(backup_path: Path, target_path: Path) -> None:
    ...
    Example:
        ...
        >>> backup = Path(".devsync/backups/20251109_143052/company.test.md")
